...
double m_speed = 13.33;
 ...
 /*                                                                           */
 /*                                                                           */
 /* 3 technologies available : 80211p, 80211b, wave                           */  
 /*****************************************************************************/
...
int compteurSwitchTechno ;
int compteurSwitchNeighbour ;
...
public:
        TechnoUsed() = default;
	TechnoUsed (uint32_t number, double throughput) {
		this->number = number ;
...
public:
  ...
  EmitterReceiver(int emitter, int receiver, int packetSize) {
    ...
    this->msgSent = 0 ;
    this->msgReceived = 0 ;
    this->technoUsed = TechnoUsed(1,6) ;
  }
  ...
  EmitterReceiver(int emitter, 
    ...
    this->technoUsed = technoUsed ;
    this->isTechnoUsed=isTechnoUsed;
    this->isNodeElecteds1 = isNodeElecteds1 ;
    this->isNodeElecteds2 = isNodeElecteds2 ;
    this->w1=w1;
    this->w2=w2;
  }
...
/*********************FUNCTIONS********************************************/
int nbUsed(uint32_t numTechnoUsed);//number of nodes using a techno numTechnoUsed
double shareThroughput(uint32_t numTechnoUsed);//shared throughput with fairness rules
void recapitulatifSources(); //output related to the choice of neighbors
void recapitulatifTechno(); //output related to the choice of technology

//algo RAT switch techno
bool testC1(uint32_t nEmitter, uint32_t potentialTechno, uint32_t actualTechno);//first condition
bool testC2(uint32_t nEmitter, uint32_t potentialTechno);//second condition
bool testC3(); //third condition
void switchTechno(uint32_t nEmitter, uint32_t lastTechno, uint32_t newTechno);

//algo switch neighbors
void updateW();//update function w
void updateY(uint32_t receiver);//update function y
void updateQ(uint32_t receiver);//update function q
void neighbourChoice(uint32_t nReceiver, uint32_t nEmitter, uint32_t techno);

//traffic file
static void GenerateTraffic (uint32_t nReceiver, uint32_t nEmitter, uint32_t techno, Ptr<Socket> socket, uint32_t pktSize, double pktInterval );
...
int nbUsed(uint32_t numTechnoUsed)
{
        int nb=0;
        for (std::vector<EmitterReceiver>::iterator it = m_vlinks.begin() ; it != m_vlinks.end() ; it ++) 
        {    
                if ((it->GetTechnoUsed().GetNumber()==numTechnoUsed) 
                        && (it->getIsTechnoUsed()))   
                {
                        nb++;               
                }
        }
        return nb/2; //2 sources     
}

double shareThroughput(uint32_t numTechnoUsed)
{
        int nb= nbUsed(numTechnoUsed);
        double throughput= 0;
        for (std::vector<EmitterReceiver>::iterator it = m_vlinks.begin() ; it != m_vlinks.end() ; it ++) {
                if (it->GetTechnoUsed().GetNumber()==numTechnoUsed)  
                {
                        throughput =it->GetTechnoUsed().GetThroughput(); 
                }
                if ((it->GetTechnoUsed().GetNumber()==numTechnoUsed)&&(nb>0))  
                {
                        return ((it->GetTechnoUsed().GetThroughput())/nb);       
                }
        }
        return throughput;      
}

void recapitulatifSources()
{ 
        cout <<"Bienvenue dans récapitulatif avec nombre de changements :"<< compteurSwitchNeighbour<<endl;
        cout <<"Bienvenue dans récapitulatif Q Source 1 :"<<endl;
        for (unsigned i=0; i<vecQ1.size(); i++)
        {
                cout<<"--["<< i<< "] | "; 
                for (unsigned j=0; j<vecQ1[i].size(); j++)
                {
                        cout<<vecQ1[i][j]<<"("<<j<<") | ";
                }
                cout<<endl;
        }
        cout <<"Bienvenue dans récapitulatif Y Source 1 :"<<endl;
    
        for (unsigned i=0; i<vecY1.size(); i++)
        {
                cout<<"--["<< i<< "] | "; 
                for (unsigned j=0; j<vecY1[i].size(); j++)
                {
                        cout<<vecY1[i][j]<<"("<<j<<")| ";
                }
                cout<<endl;
        }
        cout <<"Bienvenue dans récapitulatif Q Source 2 :"<<endl;
        for (unsigned i=0; i<vecQ2.size(); i++)
        {
                cout<<"--["<< i<< "] | ";  
                for (unsigned j=0; j<vecQ2[i].size(); j++)
                {
                        cout<<vecQ2[i][j]<<"("<<j<<") | ";
                }
                cout<<endl;
        ...
        cout <<"Bienvenue dans récapitulatif Y Source 2 :"<<endl;
    
        for (unsigned i=0; i<vecY2.size(); i++)
        {
                cout<<"--["<< i<< "] | "; 
                for (unsigned j=0; j<vecY2[i].size(); j++)
                {
                        cout<<vecY2[i][j]<<"("<<j<<") | ";
                }
                cout<<endl;
...
void recapitulatifTechno()
{
        cout <<"Bienvenue dans récapitulatif avec "<< compteurSwitchTechno<<endl;
        for (std::vector<EmitterReceiver>::iterator it = m_vlinks.begin() ; it != m_vlinks.end() ; it ++) 
        {
                if (it->getIsTechnoUsed())
                {
                        cout <<it->GetEmitter ()<<"->"<<it->GetReceiver () 
                                << " : "<<it->GetTechnoUsed ().GetNumber()
                                << "élue, nb d'utilisateur : "<<nbUsed(it->GetTechnoUsed ().GetNumber())
                                << " noeuds donc debit "<<shareThroughput(it->GetTechnoUsed ().GetNumber())
                                <<"/"<<it->GetTechnoUsed().GetThroughput()<<endl;
                }
                else
                {
                        cout <<it->GetEmitter ()<<"->"<<it->GetReceiver () 
                                << " : "<<it->GetTechnoUsed ().GetNumber()
                                << " non elue, nb d'utilisateur : "<<nbUsed(it->GetTechnoUsed ().GetNumber())
                                << " noeuds donc debit "<<shareThroughput(it->GetTechnoUsed ().GetNumber())
                                <<"/"<<it->GetTechnoUsed().GetThroughput()<<endl;
                } 
     }
...
bool testC1(uint32_t nEmitter, uint32_t potentialTechno, uint32_t actualTechno)
{
        ...
        for (std::vector<EmitterReceiver>::iterator it = m_vlinks.begin() ; it != m_vlinks.end() ; it ++) 
        {
                ...
                {
                        ActualSharedThroughput=shareThroughput(it->GetTechnoUsed ().GetNumber());
                } 
                if (it->GetTechnoUsed ().GetNumber()==potentialTechno) 
                {
                        PotentialGlobalThroughput=it->GetTechnoUsed ().GetThroughput();
                        PotentialNbUsers=nbUsed(it->GetTechnoUsed ().GetNumber());
                }
        }
...
void switchTechno(uint32_t nEmitter, uint32_t lastTechno, uint32_t newTechno)
{      
        compteurSwitchTechno++;
        cout<<endl<<"**************************** "<<endl;
        cout<<" BIENVENUE DANS switch noeud "<<nEmitter<<" avec passage techno :"<<lastTechno<<" a "<<newTechno<< " TOTAL "<< compteurSwitchTechno<<endl;
        cout<<"**************************** "<<endl<<endl;
...
bool testC2(uint32_t nEmitter, uint32_t potentialTechno)
{
        ...
        for (std::vector<EmitterReceiver>::iterator it = m_vlinks.begin() ; it != m_vlinks.end() ; it ++) {
                if (it->GetTechnoUsed ().GetNumber()==potentialTechno) 
                { 
                        PotentialshareThroughput=shareThroughput(it->GetTechnoUsed ().GetNumber());
                }
        }
...
/*Mise à jour du débit correspondant à chaque source*/
void updateW()
{
        uint32_t emitter =0;
        uint32_t receiver =0;
        ...
        for (std::vector<EmitterReceiver>::iterator it = m_vlinks.begin() ; it != m_vlinks.end() ; it ++) 
        {
                if  (it->getIsNodeElectedS1())
                {
                        ...
                        for (std::vector<EmitterReceiver>::iterator jt = m_vlinks.begin() ; jt != m_vlinks.end() ; jt ++) {
                                if  ((jt->getIsNodeElectedS2())&&(jt->GetEmitter() ==emitter)&&(jt->GetReceiver() ==receiver))
                                {
                                        twin =true ;
                                }
                        }
        ...
        for (std::vector<EmitterReceiver>::iterator it = m_vlinks.begin() ; it != m_vlinks.end() ; it ++) 
        {
                if  (it->getIsNodeElectedS2())
                {
                        ...
                        for (std::vector<EmitterReceiver>::iterator jt = m_vlinks.begin() ; jt != m_vlinks.end() ; jt ++) 
                        {
                                if  ((jt->getIsNodeElectedS1() )&&(jt->GetEmitter() ==emitter)&&(jt->GetReceiver() ==receiver))
...
void updateQ(uint32_t receiver)
{
        ...
        {  
                ...
                {
                        tabY[(int)emitter]= vecY1[(int)receiver][(int)emitter];       
                }
                j++;
        }
        double sumExpoY=0;
        ...
        for (std::vector<EmitterReceiver>::iterator i = m_vlinks.begin() ; i != m_vlinks.end() ; i ++) 
        {
                if (receiver == i->GetReceiver())
                {
                        ...
                        {
                                vecQ1[(int)receiver][k]= exp(vecY1[(int)receiver][k])/sumExpoY;
                        }
                }
                j++;
        }  
     ...
     for (std::vector<EmitterReceiver>::iterator i = m_vlinks.begin() ; i != m_vlinks.end() ; i ++) 
     {
        ...
        if (receiver == i->GetReceiver())
        {
                tabY[emitter]= vecY2[(int)receiver][(int)emitter];
        }
        j++;
     }
    ...
    for (std::vector<EmitterReceiver>::iterator i = m_vlinks.begin() ; i != m_vlinks.end() ; i ++) 
    {
        if (receiver == i->GetReceiver())
        {
                for (int k = 0; k < (int) m_nNodes; k++)
                {   
                        vecQ2[(int)receiver][k]= exp(vecY2[(int)receiver][k])/sumExpoY;
                }
        }
        j++;
     }       
...
void neighbourChoice(uint32_t nReceiver, uint32_t nEmitter, uint32_t techno)
{
     ...
     for (std::vector<EmitterReceiver>::iterator i = m_vlinks.begin() ; i != m_vlinks.end() ; i ++, j++) {
        ...
        {
                tabY[emitter]= vecQ1[(int)receiver][(int)emitter];
        }
     }
     ...
     for (std::vector<EmitterReceiver>::iterator i = m_vlinks.begin() ; i != m_vlinks.end() ; i ++, j++) {
        if ((emitter == i->GetEmitter()) && (i->getIsTechnoUsed())&&(i->getIsNodeElectedS1()))
        {
                ...
                if ((nElected == i->GetReceiver())&&(chg))
                {       
                
                        i->setIsNodeElectedS1(true);
                     compteurSwitchNeighbour++;
                       chg=false;
                }
        }
        else
     ...
     for (std::vector<EmitterReceiver>::iterator i = m_vlinks.begin() ; i != m_vlinks.end() ; i ++, j++) {
        ...
        if (emitter == i->GetEmitter())
        {
                tabY[emitter]= vecQ2[(int)receiver][(int)emitter];
        }
     }
                             ...
                             double pktInterval ) 
{
        ...
        cout<< "number of techno switch :"<<compteurSwitchTechno<< " and number of neighbors switch : "<<compteurSwitchNeighbour<<endl;
  
   //recapitulatifSources();
   //recapitulatifTechno();
  // Packet to be sent
  ...
  if (testC1(nEmitter, techno, 1) && testC2(nEmitter,  1) && testC3() )
  {
        switchTechno(nEmitter, techno, 1) ;
  }
  if (testC1(nEmitter, techno, 2) && testC2(nEmitter,  2) && testC3() )
  {
        switchTechno(nEmitter, techno, 2) ;
  }
...
void InstallNoProtocol () {
  ...
  address80211b.SetBase ("12.0.0.0", "255.0.0.0");
 ...
 */
void InstallApplications () {
cout<<"Bienvenue dans installApp"<<endl;
//Cretation des technologies de communication


	//TechnoUsed (int number, double throughput, int nbUsed) 
        TechnoUsed techno1= TechnoUsed(1, 6);
        TechnoUsed techno2= TechnoUsed(3, 6);
        TechnoUsed techno3= TechnoUsed(2, 6);
        
        cout<<"********************Installation : nb user t1 "<<nbUsed(techno1.GetNumber())<<endl;
      ...
      if (line.find("#") == std::string::npos && !line.empty()) {
        ...
        if ((iNodeEmit < 0) || (iNodeRec < 0)) {
          ...
          if (((uint32_t)iNodeEmit >= m_nNodes) || ((uint32_t)iNodeRec >= m_nNodes)) {
            ...
            if (iNodeEmit==0)
            {
                m_vlinks.push_back(EmitterReceiver(iNodeEmit, iNodeRec, packetSize,techno1, true, true, false,6,0)) ;
                m_vlinks.push_back(EmitterReceiver(iNodeEmit, iNodeRec, packetSize,techno2, false, true, false,6,0)) ;
                m_vlinks.push_back(EmitterReceiver(iNodeEmit, iNodeRec, packetSize,techno3, false, true, false,6,0)) ;
            }
            else
            {
                if(iNodeEmit==2)
                {
                        m_vlinks.push_back(EmitterReceiver(iNodeEmit, iNodeRec, packetSize,techno1, true, false, true,0,6)) ;
                        m_vlinks.push_back(EmitterReceiver(iNodeEmit, iNodeRec, packetSize,techno2, false, false, true,0,6)) ;
                        m_vlinks.push_back(EmitterReceiver(iNodeEmit, iNodeRec, packetSize,techno3, false, false, true,0,6)) ;
                }
                else
                {
                        m_vlinks.push_back(EmitterReceiver(iNodeEmit, iNodeRec, packetSize,techno1, true, false, false,0,0)) ;
                        m_vlinks.push_back(EmitterReceiver(iNodeEmit, iNodeRec, packetSize,techno2, false, false, false,0,0)) ;
                        m_vlinks.push_back(EmitterReceiver(iNodeEmit, iNodeRec, packetSize,techno3, false, false, false,0,0)) ;
                }
...
/* ********************************************************************************************* */
int main (int argc, char *argv[]) {
        compteurSwitchTechno=0;
        compteurSwitchNeighbour=0;
        srand(time(NULL));
